...
LoadUserspaceElf64Process(
    ...
    for (
        ...
        if (phdr->p_memsz == 0)
            std::print("[ELF]: Warning: program header has zero size.\n");
...
ReplaceUserspaceElf64Process(Process* process, ProcessFileDescriptor fd, const std::vector<std::string_view>& args = {}) {
    ...
    for (const auto& region : process->Memories) {
        Memory::unmap_pages(process->CR3, region.vaddr, region.pages);
...
inline bool
CreateUserspaceElf64Process(ProcessFileDescriptor fd, const std::vector<std::string_view>& args) {
    if (!args.size()) {
