...
__attribute__((no_caller_saved_registers)) void printerrupt(std::format_string<_Args...> __fmt, _Args&&... __args) {
...
__attribute__((no_caller_saved_registers)) static void handle_direct_input(char input) {
    if (!input) {
...
__attribute__((interrupt)) void page_fault_handler(InterruptFrameError* frame) {
    ...
    */
    ...
    if (frame->error & (u64)PageFaultErrorCode::UserSuper)
        ...
        Memory::print_page_map((Memory::PageTable*)cr3);
    ...
    if ((frame->error & (u64)PageFaultErrorCode::UserSuper) > 0) {
        if ((frame->error & (u64)PageFaultErrorCode::ReadWrite) > 0) {
            // FIXME: Could probably do without the heap allocation from kernel format but, whatevz
            if (notPresent)
        ...
        if ((frame->error & (u64)PageFaultErrorCode::ReadWrite) > 0) {
            ...
            if (notPresent)
                ...
                panic(frame, "#PF: Supervisor process attempted to read from a page and caused a protection fault");
        }
    }
...
__attribute__((interrupt)) void double_fault_handler(InterruptFrameError* frame) {
    panic(frame, "Double fault detected!");
    hang();
}

__attribute__((interrupt)) void stack_segment_fault_handler(InterruptFrameError* frame) {
    if (frame->error == 0)
