...
int main(int argc, char** argv) {
    ...
    int status = 0;
    // FIXME: This waits for there not to be an error; technically, we should
    // either block until there is an event in the syscall itself, or wait
    // until some sign that events have been written on the userspace side of
    // things.
    ...
    int id = syscall(SYS_shared_memory_allocate, &shared_data, 4096);
    printf("id:%d data:%p\n", id, shared_data);
    ...
    printf("[SERVE]: first 8-byte of shared data: %u\n", *shared_data);
    fflush(stdout);
